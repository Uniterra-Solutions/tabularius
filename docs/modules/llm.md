...
|--------|-----------|-------|
| `__init__` | `(*, api_key, base_url, model, max_tokens, timeout, client)` | `client` injects a pre-built OpenAI SDK client (tests); otherwise builds one from env key |
| `chat` | `(messages, tools=None, *, max_tokens=None, timeout=None) -> response` | Always sends `response_format={"type": "json_object"}`; adds `tools` when provided; retries 429/5xx/timeout |
